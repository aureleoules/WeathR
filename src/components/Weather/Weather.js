...
class Weather extends React.Component {
    ...
    refreshMeteo = () => {
        ...
        for (var i = 0; i < this.state.cities.length; i++) {
            ...
            fetch(`http://api.openweathermap.org/data/2.5/weather?q=${city}&units=metric&APPID=61137e48c84896f2f196f1788a7d9592`).then(function(response) {
                ...
                meteo.push(data);
                if (meteo.length === self.state.cities.length) {
                    self.setMeteo(meteo);
                }
            });
        }
    ...
    getTemp = (key) => {
        if (this.state.meteo[key]) {
            return this.state.meteo[key].weather[0].main;
        }
        return 'LOADING...';
    }
    ...
    render() {
        ...
        const items = Object.keys(this.state.cities).map(key => <WeatherItem key={key} title={this.state.cities[key]} iconName={this.getIcon(key)} subtitle={this.getTemp(key)} ref={this.state.cities[key]} deleteBtn={() => {
